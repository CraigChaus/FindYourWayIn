function createNewLocation(res: any, i: any, label1: string, label2: any) {
    const newLocation = {
        ...
        xcoordinate:
            res.results[i].location.address.gisCoordinates[0].xcoordinate,
        ycoordinate:
            res.results[i].location.address.gisCoordinates[0].ycoordinate,
    };
    return newLocation;
}
    ...
    await fetch('https://app.thefeedfactory.nl/api/locations?size=34', {
        ...
        .then((res) => {
            for (let i = 0; i < res.size; i++) {
                ...
                switch (
                    res.results[i].trcItemCategories.types[0]
